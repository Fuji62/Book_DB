...
%>
	<tr>
	<td><input type="hidden" name="isbn<%=cnt %>" value="<%=rs.getString("isbn") %>" />
		<input type="checkbox" name="add<%=cnt %>" value="1" size="3"></td>
	<td><input type="text" name="title<%=cnt %>" value="<%=rs.getString("title") %>" size="30" /></td>
	<td><input type="text" name="stocks<%=cnt %>" value="<%=rs.getString("stocks") %>" size="5" /></td>
	<td><input type="text" name="num_of_items<%=cnt %>" placeholder="購入数を入力" size="10" /></td>
	</tr>
<%
			}
		}catch(Exception e){
